...
display: none;
  ...
  <div class="col-9 col-sm-10" style="padding-top: 67px; background-image: linear-gradient(120deg, #fdfbfb 0%, #ebedee 100%); border-radius: 0px 15px 0px 5px;">
     <h3 class="p-2 p-sm-4 p-sm-4 display-5" style="font-size: 25px;"  >
        ...
        <div class="col-12 col-sm-7 mt-3 mb-5 row justify-content-center" style="box-shadow: rgba(149, 157, 165, 0.2) 0px 8px 24px; border-radius: 15px;">
            ...
            //Añade el seguimiento realizado en el comentario enviado por post
                ...
                while( $row = mysqli_fetch_assoc($query_C)){
                        ...
                        if($row_e['COD_USUARIO_SOLICITA'] == $row['COD_USUARIO_SEGUI']){    //validacion para diferenciar los seguimientos, zona izquierda quien solicita y derecha quien atiende el caso. 
                ...
                $evaluar = '';
                    ...
                    if ($row_e['ESTADO'] == 'Cerrado'){
                            ...
                            switch ($row_k['CALIFICACION']) {
                                case 1:
                                    $evaluar = "<label for='radio1' style='color: orange;'>★</label>";
                                    break;
                                case 2:
                                    $evaluar = "<label for='radio1'>★</label><label for='radio1'>★</label>";
                                    break;
                                case 3:
                                    $evaluar = "<label for='radio1'>★</label><label for='radio1'>★</label><label for='radio1'>★</label>";
                                    break;
                                case 4:
                                    $evaluar = "<label for='radio1'>★</label><label for='radio1'>★</label><label for='radio1'>★</label><label for='radio1'>★</label>";
                                    break;
                                case 5:
                                    $evaluar = "<label for='radio1'>★</label><label for='radio1'>★</label><label for='radio1'>★</label><label for='radio1'>★</label><label for='radio1'>★</label>";
                                    break;
                            }
                        }
...
?>
         ...
         label {
            ...
            <form class="row my-2" method="post" action=""  enctype="multipart/form-data" style="background-color: #E5E8E8; border-radius: 5px;">
                <div class="m-1 m-sm-4 row">
                    <div class="m-2 px-3 col-6 d-flex align-items-center">
                        

                        Encuesta satisfaccion: &nbsp; <?php echo $evaluar ; ?> &nbsp; <button type="button" class="btn btn-light" data-bs-toggle="modal" data-bs-target="#encuesta_user">  <i class="bi bi-chat-right-text"></i></button>

                         <!-- Modal encuesta-->

                        <div class="modal fade" id="encuesta_user" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                            <div class="modal-header">
                                <h1 class="modal-title fs-5" id="exampleModalLabel">Comentario del usuario</h1>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                    <?php

                         if ($row_k) { echo $row_k['SUGERENCIA']; } else { echo "N/A";}  
       ...
       <div class="col-12 col-md-3 mb-5 row mx-5 h-50" style="box-shadow: rgba(149, 157, 165, 0.2) 0px 8px 24px; border-radius: 15px; border: 1px solid rgba(149, 157, 165, 0.1); height: 180px;">
       <div class="mt-2" style="font-size: 16px;" >
        Fecha apertura: <?php echo $row_e['FECHA_CREACION_CASO'] ?><br> 
        Cierre del caso: <?php echo $row_e['FECHA_FINALI_CASO'] ?>
       </div>
            <div class="mt-2">
                <p class="display-6" style="font-size: 26px; font-weight: 350;">Persona a cargo del caso</p>
            </div>
            <div class="col-3 col-sm-3 h-80 text-center">
                 <i class="bi bi-person-circle" style="font-size:50px;"></i>
            </div>
            <div class="col-9 row align-items-center">
                <p class="display-5" style="font-size:15px;"> <strong>Nombre: </strong> <?php if ($row_e){ echo $row_e['NOMBRE_USUARIO']." ".$row_e['APELLIDO']; }else{ echo "El caso se encuentra en cola";}?> <br>
                    <strong>Cargo: </strong> <?php if ($row_e) {echo $row_e['CARGO'];} else { echo "-";} ?><br>
                    <strong>Correo: </strong> <?php if ($row_e) {echo $row_e['CORREO'];} else{ echo "-";} ?>
                </p>
            </div>
            <div class="mt-2">
...
<?php
            ...
            $consult_z = "SELECT * FROM baseuno.casos
                ...
                $query_z =  mysqli_query($db, $consult_z);

                $row_z = mysqli_fetch_assoc($query_z);

?>

            <div class="col-9 row align-items-center">
                <p class="display-5" style="font-size:15px;"><strong>Nombre: </strong><?php echo $row_z['NOMBRE_USUARIO']." ".$row_z['APELLIDO'] ?> <br>
                    <strong>Cargo: </strong> <?php echo $row_z['CARGO'] ?><br>
                    <strong>Correo: </strong> <?php echo $row_z['CORREO'] ?><br>
                    <strong>Dependencia: </strong> <?php echo $row_z['DEPENDENCIA'] ?><br>
                    <strong>Ubicacion: </strong> <?php echo $row_z['UBICACION'] ?><br>
                </p>
            </div> 
            <div class="my-1" style="max-height: 370px; overflow-y: scroll;">
            <p class="display-6 overflow-y: scroll;" style="font-size: 26px; font-weight: 350;">Descripcion del caso #<?php echo $row_e['COD_CASO'] ?></p> <p class="text-center col-4" style="border-radius:15px;  background-color:<?php echo $estado_caso; ?>;"> <?php if ($row_e){ echo "Caso ".$row_e['ESTADO']; }else{ echo "-"; } ?></p>
